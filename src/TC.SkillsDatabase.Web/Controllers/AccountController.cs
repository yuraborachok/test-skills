namespace TC.SkillsDatabase.Web.Controllers
{
    ...
    public class AccountController : BaseAbstractController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl, bool captchaValid)
        {
            ...
            var user = this.UserManager.Find(model.Email, model.Password);
            ...
            if (user != null)
            {
                if (!await this.UserManager.IsEmailConfirmedAsync(user.Id))
                {
                    ...
                    this.ViewBag.errorMessage = "You must have a confirmed email to log on. The confirmation token has been resent to your email account.";
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, bool captchaValid)
        {
            ...
            if (this.ModelState.IsValid)
            {
                ...
                user.Claims.Add(new CustomUserClaim { ClaimType = ClaimTypes.GivenName, ClaimValue = user.FirstName });
                ...
                if (result.Succeeded)
                {
                    ...
                    this.ViewBag.Message = "Registered successfuly";
        ...
        private async Task<string> SendEmailConfirmationTokenAsync(int userID, string subject)
        {
            ...
            var callbackUrl = this.Url.Action("ConfirmEmail", "Account", new { userId = userID, code = code }, protocol: this.Request.Url.Scheme);
            await this.UserManager.SendEmailAsync(userID, subject, string.Format("Thanks for registering to the Thomas Cook Champions awards portal.<br />Please just click on the following link to confirm your registration.<br /><a href=\"{0}\">{0}</a>", callbackUrl));
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
            ...
            if (e is HttpAntiForgeryException && e.Message.Contains("The anti-forgery token could not be decrypted."))
            {
                filterContext.ExceptionHandled = true;
                this.ViewBag.Message = "Session expired";
                filterContext.Result = this.View("Info");
            }
        }
    }
}